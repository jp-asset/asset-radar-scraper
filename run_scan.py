...
from core.sources_config import load_enabled_sources, is_enabled
...
def run_full_scan() -> dict:
    ...
    for cls in ALL_ACTOR_SCRAPERS:
        if not is_enabled(cls.portal_name, enabled):
            log.info(f"[{cls.portal_name}] ignorado (desactivado em config/sources.json)")
            source_stats.append({
                "portal": cls.portal_name, "found": 0,
                "method": "apify_actor", "skipped": True,
            })
            continue
        scraper = cls()
        try:
    ...
    try:
        http_results = run_all_http_scrapers(ZONES, enabled)
        for portal_name, listings in http_results:
            if listings is None:  # ignorado via config/sources.json
                source_stats.append({
                    "portal": portal_name, "found": 0,
                    "method": "http_directo", "skipped": True,
                })
                continue
            for l in listings:
                compute_market_estimate(l)
...
def main():
    ...
    for s in result["source_stats"]:
        if s.get("skipped"):
            status = "ignorado (desactivado em config/sources.json)"
        elif s.get("error"):
            status = f"erro: {s['error']}"
        else:
            status = f"{s['found']} anúncios"
